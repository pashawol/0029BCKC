...
const JSCCommon = {
	...
	inputMask() {
		...
		let maskedDate = [].slice.call(document.querySelectorAll('.masked-date'));
                let InputTelId = document.querySelectorAll('#reg-uf_phone')[0];
		InputTel.forEach(element => element.setAttribute("pattern", "[+][0-9]{1}[(][0-9]{3}[)][0-9]{3}-[0-9]{2}-[0-9]{2}"));
                
                if (InputTelId) {
                    InputTelId.setAttribute("pattern", "[+][0-9]{1}[(][0-9]{3}[)][0-9]{3}-[0-9]{2}-[0-9]{2}");
                    Inputmask("+7(999)999-99-99").mask(InputTelId);
                }
...
function eventHandler() {
	...
	let sHeroSlider = new Swiper('.sHero-slider-js', {
		...
		navigation: {
			nextEl: '.sHero__card-row-next',
			prevEl: '.sHero__card-row-prev',
		},
		breakpoints: {
     			400: {
				slidesPerView: 1,
	...
	FilePond.create(
		document.querySelector('.load-photo-ticket--js input'),
		{
	...
	FilePond.create(
		document.querySelector('.load-photo-ticket--js-2 input'),
		{
	...
	FilePond.create(
		document.querySelector('.load-photo-ticket--js-3 input'),
		{
	...
	$('.toggle-text-volunteers').click(function (e) {
		e.preventDefault();
		let text = $(this).siblings('.toggle-text')
		let attr = text.attr("style");
		if (typeof attr !== typeof undefined && attr !== false) {
			$(this).html("подробнее...")
			text.removeAttr("style");
